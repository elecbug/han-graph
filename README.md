...
현재는 한자 1,949개(독음 레코드 2,062개)와 단어 2,541개(성구 포함)를 바탕으로 단어·한자 탐색, 연결 그래프, 단어장과 문맥 연습 6문항을 제공하는 로컬 웹 프로그램 및 Go CLI 단계입니다. 첫 610개 한자에 연결 단어를 최소 5개씩 채웠으며, 앞으로는 한 번에 20개 한자씩 확장합니다. 내역은 [데이터 작업 기록](./dataset/README.md)에 있습니다.
웹 프로그램의 기능과 실행 방법은 [학습 웹 앱 문서](./docs/WEB_APP.md)를 참조하세요.
프로젝트 현황과 다음 개발 순서는 [개발 문서](./docs/README.md), 데이터 형식은 [데이터 모델](./docs/DATA_MODEL.md)을 참조하세요.

Go 1.26.2 이상에서 저장소 루트를 기준으로 실행합니다.

```sh
cd src
go run ./cmd serve
# 브라우저에서 http://127.0.0.1:18080 접속
```

CLI 조회와 검증은 `src`에서 실행합니다.
...
데이터 경로를 바꾸려면 명령 앞에 `-data <디렉터리>`를 지정합니다. 아래의 프로그램 설명에는 장기 목표도 포함됩니다. 현재 구현 범위와 남은 과제는 개발 문서를 참조하세요.
