...
현재는 한자 1,817개와 단어 20개를 바탕으로 데이터 검증, 단어 분해, 한자별 연결 단어 조회를 제공하는 Go CLI 단계입니다.
...
데이터 경로를 바꾸려면 명령 앞에 `-data <디렉터리>`를 지정합니다. 아래의 학습 화면, 문맥 퀴즈, 네트워크 시각화 등은 장기 목표이며 아직 구현되지 않았습니다.
