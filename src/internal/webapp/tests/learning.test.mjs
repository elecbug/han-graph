...
test('each round samples ten distinct questions from the whole pool and shuffles options',()=>{
  const pool=Array.from({length:100},(_,index)=>({id:`q-${index}`,answer:fraction,options:[fraction,fountain]}));
  ...
  assert.deepEqual(first.questions[0].options,[fountain,fraction]);
  assert.deepEqual(second.questions[0].options,[fraction,fountain]);
...
test('practice pools with ten or fewer questions use every available question once',()=>{
  for(const size of [0,1,6,10]) {
    const pool=Array.from({length:size},(_,index)=>({id:`small-${index}`,answer:fraction,options:[fraction,fountain]}));
