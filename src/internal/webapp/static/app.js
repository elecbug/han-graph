import {wordKey, normalizeSaved, createSession, answerQuestion, sessionResult, PRACTICE_SESSION_SIZE} from './learning.mjs';
...
function renderPractice() {
  ...
  if(!state.session) {
    const poolSize=state.practice.questions.length;
    ...
    $('#main').innerHTML=`${head}<div class="practice-wrap"><section class="practice-intro"><div class="practice-art" aria-hidden="true"><span>文</span><i>＋</i><span>脈</span></div><h2>${t('introTitle')}</h2><p>${t('introSub')}</p><div class="practice-steps"><span><b>1</b>${t('step1')}</span><span><b>2</b>${t('step2')}</span><span><b>3</b>${t('step3')}</span></div><button class="primary-button" data-action="start-practice">${t('startPractice')} →</button><p class="practice-meta">${esc(roundLabel)} · ${t('practiceDraft')}</p>${validLast?`<p class="practice-meta">${t('latest')}: ${last.correct} / ${last.total}</p>`:''}</section></div>`;
    return;
  }
