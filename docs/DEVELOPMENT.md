...
변경 범위에 맞춰 다음 검사를 실행한다. 셸에서 테스트 폴더를 지정하면 Node가 그 안의 테스트 파일을 찾는다.

```sh
go run ./cmd validate
go test -count=1 ./...
go vet ./...
node --test internal/webapp/tests
```

| 검사 | 주요 범위 |
| --- | --- |
| 데이터 검증 CLI | 세 JSONL 파일의 형식·중복·참조와 그래프 수치 |
| Go 테스트 | 조회·무작위 선택·그래프 범위, 연습 참조, API·초기 HTML·캐시 헤더 |
| JavaScript 테스트 | 조회 캐시, 단어장 파일, 채점, 그래프 배치·연결선·이동 |
| 브라우저 점검 | 실제 탐색, 언어 전환, 저장·새로고침, 확대·드래그·키보드, 모바일 배치 |

그래프 테스트는 데이터의 모든 단어를 순회하며 상자 겹침, 좌표·경로 잘림, 연결선의 다른 상자 관통을 검사한다. 데이터가 늘면 검사 시간도 늘어날 수 있다.

화면을 바꿨을 때는 데스크톱과 약 390px 모바일 너비에서 확인한다. 단어·한자 이동, 동음이의어 선택, 단어장 저장과 파일 왕복, 연습 완료, 그래프 클릭·확대·이동과 페이지 스크롤을 점검한다.
...
```sh
node --preserve-symlinks --preserve-symlinks-main --test internal/webapp/tests
```
